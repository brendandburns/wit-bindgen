use anyhow::Result;
use heck::{ToLowerCamelCase, ToShoutySnakeCase, ToPascalCase};
use std::{
...
impl WorldGenerator for Dotnet {
    ...
    fn finish(&mut self, resolve: &Resolve, id: WorldId, files: &mut Files) {
        ...
        for &count in &self.tuple_counts {
            ...
            uwrite!(
                src,
                "
                public static sealed class Tuple{count}{type_params} {{
                    {fields}
        ...
        if self.needs_result {
            src.push_str(
                r#"
                public static sealed class Result<Ok, Err> {
        ...
        if self.needs_cleanup {
            src.push_str(
                "
                public static sealed class Cleanup {
        ...
        files.push(&format!("{directory}/{name}.cs"), indent(&src).as_bytes());
...
impl InterfaceGenerator<'_> {
    ...
    fn import(&mut self, _module: &str, func: &Function) {
        ...
        let mut bindgen = FunctionBindgen::new(
            ...
            func.params
                .iter()
                .map(|(name, _)| name.to_dotnet_ident())
                .collect(),
        );
        ...
        let pascal_name = func.name.to_pascal_case();
        ...
        uwrite!(
            self.src,
            r#"[MethodImpl(MethodImplOptions.InternalCall)]
               private unsafe static extern {result_type} WasmImport{pascal_name}({params});
    ...
    fn export(&mut self, interface_name: Option<&str>, func: &Function) {
        ...
        uwrite!(
            ...
            private static {result_type} WasmExport{pascal_name}({params}) {{
                {src}
            }}
            "#
        );
        ...
        if self.resolve.guest_export_needs_post_return(func) {
            ...
            uwrite!(
                ...
                private static void wasmExport{pascal_name}PostReturn({params}) {{
                    {src}
                }}
                "#
            );
    ...
    fn sig_string(&mut self, func: &Function, qualifier: bool) -> String {
        let name = func.name.to_dotnet_ident().to_pascal_case();
...
impl<'a> wit_bindgen_core::InterfaceGenerator<'a> for InterfaceGenerator<'a> {
    ...
    fn type_record(&mut self, _id: TypeId, name: &str, record: &Record, docs: &Docs) {
        ...
        let assignments = record
            ...
            .iter()
            .map(|field| {
                let name = field.name.to_dotnet_ident();
    ...
    fn type_variant(&mut self, _id: TypeId, name: &str, variant: &Variant, docs: &Docs) {
        ...
        let constructors = variant
            ...
            .iter()
            .map(|case| {
                let case_name = case.name.to_dotnet_ident();
    ...
    fn lift_variant(
        ...
        let cases = cases
            ...
            .map(|(i, ((case_name, case_ty), Block { body, results, .. }))| {
                ...
                let method = case_name.to_dotnet_ident();
        ...
        match inst {
            ...
            Instruction::RecordLower { record, .. } => {
                ...
                for field in record.fields.iter() {
                    results.push(format!("({op}).{}", field.name.to_dotnet_ident()));
                }
            }
            ...
            Instruction::ListCanonLift { element, .. } => {
                ...
                let ty_upper = ty.to_pascal_case();
            ...
            Instruction::CallWasm { sig, .. } => {
                ...
                let func_name = self.func_name.to_pascal_case();
            ...
            Instruction::CallInterface { func, .. } => {
                ...
                let name = func.name.to_dotnet_ident();
...
fn world_name(resolve: &Resolve, world: WorldId) -> String {
    format!(
        "wit.worlds.{}",
        resolve.worlds[world].name.to_pascal_case()
    )
}
...
fn interface_name(resolve: &Resolve, name: &WorldKey, direction: Direction) -> String {
    ...
    format!(
        ...
        if let Some(name) = &pkg {
            format!(
                "{}.{}.",
                name.namespace.to_dotnet_ident(),
                name.name.to_dotnet_ident()
            )
        } else {
            String::new()
        }
...
trait ToDotnetIdent: ToOwned {
    fn to_dotnet_ident(&self) -> Self::Owned;
}

impl ToDotnetIdent for str {
    fn to_dotnet_ident(&self) -> String {
